...
public class QuickSmelt {
    public static void quickSmelt(Player player, Economy econ, PlayerInteractEvent event) {
        ...
        if (im != null){
            if (im.getLore() != null){
                String quickSmeltLore = im.getLore().get(0);
                if (quickSmeltLore.equalsIgnoreCase("QuickSmelt")){
                    if (event.getAction().toString().equals("LEFT_CLICK_BLOCK") && event.getPlayer().getInventory().getItemInMainHand().getType().equals(Material.STICK) && event.getClickedBlock().getType().equals(Material.FURNACE)) {
