...
public class DenyInteractForUnlinkedPlayerEvent implements Listener {
    public DenyInteractForUnlinkedPlayerEvent(Main plugin) {plugin.getServer().getPluginManager().registerEvents(this, plugin);}

    @EventHandler(priority = EventPriority.HIGHEST)
    public void onPlayerInteract(PlayerInteractEvent e) {
        FileConfiguration enabledEventsCfg = Api.getFileConfiguration(YmlFileNames.YML_ENABLED_EVENTS_FILE_NAME);
        if (enabledEventsCfg.getBoolean("disableInteractAndSendMessageToUnlinkedPlayer")) {
