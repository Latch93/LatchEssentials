...
public class Api {
    ...
    public static void checkPlayerMemberStatus(Player player){
        ...
        assert user != null;
        if (!"default".equalsIgnoreCase(user.getPrimaryGroup()) && Boolean.TRUE.equals(Api.getFileConfiguration(Api.getConfigFile(YmlFileNames.YML_CONFIG_FILE_NAME)).getBoolean("showJoinMessage"))){
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', Objects.requireNonNull(Objects.requireNonNull(Api.getFileConfiguration(Api.getConfigFile(YmlFileNames.YML_CONFIG_FILE_NAME)).getString("joinMessage")))));
        }
    ...
    public static void placeBlockLog(BlockPlaceEvent event) throws IOException {
        if ((event.getBlock().getLocation().getWorld().getName().equalsIgnoreCase("world") ) && (event.getBlock().getType().equals(Material.TNT) || event.getBlock().getType().equals(Material.END_CRYSTAL))){
            FileConfiguration blockPlaceLogCfg = Api.getFileConfiguration(Api.getConfigFile(YmlFileNames.YML_BLOCK_PLACE_LOG_FILE_NAME));
            Date date = new Date();
    ...
    public static void blockBreakLog(BlockBreakEvent event) throws IOException, ExecutionException, InterruptedException {
        ...
        if (materialBreakList.contains(event.getBlock().getType())) {
            if (!Api.doesPlayerHavePermission(event.getPlayer().getUniqueId().toString(), "helper")) {
                FileConfiguration blockBreakLogCfg = Api.getFileConfiguration(Api.getConfigFile(YmlFileNames.YML_BLOCK_BREAK_LOG_FILE_NAME));
                Date date = new Date();
                Player player = event.getPlayer();
...
//        for (String idNotInDiscord : )
    ...
    public static void removePlayerFromDonationList(ArrayList<String> minecraftIDListToRemove) throws IOException {
        FileConfiguration donationClaimCfg = Api.getFileConfiguration(Api.getConfigFile(YmlFileNames.YML_USER_DONATION_REWARD_FILE_NAME));
    ...
    public static void updateDiscordRolesFile(){
        FileConfiguration discordRoleCfg = Api.getFileConfiguration(Api.getConfigFile(YmlFileNames.YML_DISCORD_ROLES_FILE_NAME));
